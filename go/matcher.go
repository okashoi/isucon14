...
func matchingOne() error {
	...
	// マンハッタン距離が最も近い椅子
	if err := db.Get(&matchedChairId, "SELECT l.chair_id FROM latest_chair_locations AS l WHERE l.chair_id IN (SELECT id FROM chairs WHERE is_active = 1 AND id NOT IN (SELECT chair_id FROM rides WHERE evaluation IS NULL)) ORDER BY ABS(latest_latitude - ?) + ABS(latest_longitude - ?) LIMIT 1", ride.PickupLatitude, ride.PickupLongitude); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
